...
/** A5 — trending (chrome i18n; story headlines stay CMS). */
export function TrendingPageView({ articles, topicChips = [] }: Props) {
  ...
  const stories = articles.slice(0, 20).map((a, i) => {
    ...
    return {
      ...
      growthLabel: i < 5 && a.ranking?.isTrending ? `▲ ${t("home.trending")}` : undefined,
    };
  });

  const chips =
    topicChips.length > 0
      ? topicChips.slice(0, 6).map((topic) => ({
          label: topic.startsWith("#") ? topic : `#${topic}`,
          href: `/search?q=${encodeURIComponent(topic.replace(/^#/, ""))}`,
        }))
      : [{ label: `#${t("home.trending")}`, href: "/trending" }];

  return (
    <ReaderShell activeNav="latest">
      <Masthead pageTitle={t("home.trending")} />
      <div
        className="jd-ui"
