...
function getCachedHomepageFeedBuild(
  ...
  return unstable_cache(
    async (): Promise<HomepageFeedBuild> => {
      ...
      const feed = await buildFeedFromPool(pool, displayLanguage, tenant);
      return {
        feed,
        diagnostics,
        freshWire: isFreshWireSource(diagnostics.source),
    ...
    ["homepage-generated-feed-v9", tenantSlug, displayLanguage],
    {
...
export async function getGeneratedHomepageFeed(): Promise<GeneratedHomepageFeed | null> {
  ...
  const cacheKey = buildHomepageFeedRedisKey(tenant.slug, displayLanguage);
  ...
  const built = await getCachedHomepageFeedBuild(tenant.slug, displayLanguage)();
  ...
  if (built.freshWire) {
    ...
    const feed = pool.length
      ? await buildFeedFromPool(pool, displayLanguage, tenant)
      : null;
    if (feed) {
