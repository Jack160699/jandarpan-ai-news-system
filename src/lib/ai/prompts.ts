...
export function buildEditorialPipelineSystemPrompt(input: {
  ...
  const correctionBlock = input.repairContext
    ? [
        ...
        `Rewrite the complete article from the fact pack. The body sections alone must contain at least ${input.repairContext.minWords} words and should approach ${input.repairContext.targetWords} when evidence supports it.`,
