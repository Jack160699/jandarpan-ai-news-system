 ...
 * GET list | POST create | PATCH role/status | DELETE remove
 */
...
export async function GET(request: Request) {
  ...
  const team = await listTenantTeamMembers(
...
export async function POST(request: Request) {
  ...
  if (!body.email || !body.password || !body.fullName || !body.role) {
    return NextResponse.json(
      { ok: false, error: "email_password_name_role_required" },
      { status: 400 }
    );
  }
...
export async function PATCH(request: Request) {
  ...
  if (body.status) {
    ...
    if (result.ok) {
      await logEditorialAudit({
        session: guard.session,
        action: body.status === "active" ? "team_reactivate" : "team_suspend",
        resourceType: "membership",
        resourceId: body.membershipId,
  ...
  return NextResponse.json(
    { ok: false, error: "role_or_status_required" },
    { status: 400 }
  );
}
