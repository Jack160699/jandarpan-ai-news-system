-- 035: Enterprise security — RLS hardening, security audit tables, RBAC helpers
...
create policy "Super admin update tenant memberships"
  ...
  with check (
    public.security_is_super_admin(tenant_id)
    and tenant_id in (select public.security_user_tenant_ids())
    -- Prevent self-escalation via direct PostgREST
    and (
      user_id <> auth.uid()
      or role = (select tm.role from public.tenant_memberships tm where tm.id = tenant_memberships.id)
    )
  );
...
grant execute on function public.match_intelligence_embeddings(vector, int, text, uuid) to service_role;
